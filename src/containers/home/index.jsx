...
export default function Home() {
    ...
    const [
        ...
        {ProductCode, MinimumEligibleLoanAmount: MinAmount, MaximumEligibleLoanAmount: MaxAmount, IntervalAmountAllowedToChange: IntervalAmount, tenureAmountList, CardNumber: cardNo, MobileNumber: mobile, DisbursalMode, SBAccountNumber, EntityName, ResponseCode} 
        = {ProductCode: 1, EntityName: 'HDFC', MinimumEligibleLoanAmount: 0, MaximumEligibleLoanAmount: 1000000, IntervalAmountAllowedToChange: 100000, tenureAmountList: [], CardNumber: 1234, MobileNumber: 9999999999, DisbursalMode: "DD", SBAccountNumber: "1234"},
        detailsStatus, detailsError,
    ] = useFetch(true);
    const [fetchOTP, otpLoading, otpResponse, otpStatus, otpError] = useFetch(true);
    ...
    const [timeLeft, startTimer, retryCount] = useCountdownTimer({
        ...
        onEnd: toggleResendOTP, 
        interval: config.initOTPRetryInterval,
        autoIncrementInterval: config.autoIncrementOTPRetryInterval,
        maxClickCount: config.maxOtpRetryAttempts,
    });

    const attempts = retryCount > 0 ? config.maxOtpRetryAttempts - retryCount : config.maxOtpRetryAttempts;
    ...
    useEffect(()=>{if(isApproved===true) incStep()}, [isApproved]);
    ...
    const submitFeedback = async() => {
        toggleLoading();
          try {
            const res = await backendServer.post(config.backendFeedbackEndpoint, {
            ...
            toggleFeedbackSubmitted();
                ...
                setError(err.response?.body?.ResponseMessage || err.ResponseMessage || `feedback submitting failed`);
          }
          toggleLoading();
    }
    ...
    return (
        detailsLoading || ResponseCode == '1' ? <Container component="main" maxWidth="xs">
            <div className={classes.paper}>
                ...
                >
                    ...
                    {detailsLoading ? (<Skeleton animation="wave" />) : <GlobalContextProvider value={{ 
                        ...
                        setApproved, isOTPPage, mobile, attempts,
        ...
        </Container> : <ErrorPage type="error" message={detailsError} showButton={false}/>
    );
}
